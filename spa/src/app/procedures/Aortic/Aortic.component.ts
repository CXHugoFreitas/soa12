...
export class AorticComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.pd.operative_technique === '3') {
            ...
            this.auth.currentProcedureId.subscribe((res) => {
                const procedure_id = +res;
                this.vs.getConduits(procedure_id).subscribe((next) => {
                    debugger;
    ...
    saveAortic() {
        this.aorticService.saveAOS(this.pd).subscribe((next)=>{this.alertify.message('saving Aortic');});
